...
// ============ CONFIGURATION ============
const API_KEY = "pGajBYIXPZUBfph1gfif9TrtJSTKtXG3Drxfs7iTtCc2mwj8kx";
...
const DEFAULTS = {
    ...
    multiplier: 1.2,
    payout: 2.0,
    balanceStep: 0.00000050, // Metric for scaling
    betIncrement: 0.00000001  // Amount to scale by
};
...
function loadState() {
    if (fs.existsSync('./bot-state.json')) {
        try {
            const data = JSON.parse(fs.readFileSync('./bot-state.json'));
            botState.profitProtection = data.profitProtection || botState.profitProtection;
        } catch(e) {}
    }
}
...
async function runStrategy() {
    ...
    while (botState.running) {
        ...
        // Apply Scaling: 0.00000150 bal -> 0.00000003 base
...
<html>
<head>
    <title>Crypto.Games Pro Bot</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { 
            ...
            color: #00ff88; padding: 20px; min-height: 100vh;
        }
        .container { max-width: 1300px; margin: 0 auto; }
        .header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; }
        .grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-bottom: 20px; }
        .card { 
            background: rgba(26, 31, 58, 0.95); padding: 20px; border-radius: 15px; 
        ...
        th { background: rgba(0, 255, 136, 0.05); color: #888; }
...
<body>
    <div class="container">
        <div class="header">
            <h1><span class="live-dot"></span> DICE PRO: ABSOLUTE SCALING</h1>
            <div style="text-align: right">
                <div class="label">Bitcoin Price</div>
                <div style="color: #ffaa00; font-weight: bold;">$${btcPrice.toLocaleString()}</div>
            </div>
        </div>

        <div class="grid">
            <div class="card safe-card">
                <div class="label">🔒 Protected Profit (50%)</div>
                <span class="btc-val" style="color:#ffaa00">${fmt(botState.profitProtection.safeBalance)}</span>
                <span class="usd-val">${usd(botState.profitProtection.safeBalance)}</span>
            </div>
            <div class="card">
                <div class="label">💰 Wallet Balance</div>
                <span class="btc-val">${fmt(botState.stats.currentBalance)}</span>
                <span class="usd-val">${usd(botState.stats.currentBalance)}</span>
            </div>
            <div class="card">
                <div class="label">📊 Session Net Profit</div>
                <span class="btc-val ${botState.stats.netProfit >= 0 ? 'win' : 'loss'}">${fmt(botState.stats.netProfit)}</span>
                <span class="usd-val">${usd(botState.stats.netProfit)}</span>
            </div>
            <div class="card">
                <div class="label">🎯 Next Bet Amount</div>
                <span class="btc-val">${fmt(botState.settings.currentBet)}</span>
                <span class="usd-val">${usd(botState.settings.currentBet)}</span>
            </div>
        </div>
        ...
        <div class="grid">
            ...
            <div class="card"><div class="label">Wins / Losses</div><div class="btc-val"><span class="win">${botState.stats.wins}</span> / <span class="loss">${botState.stats.losses}</span></div></div>
        ...
        <h3 style="margin: 30px 0 15px 0; color: #888; font-size: 14px; text-transform: uppercase;">📈 Estimated Profit Projections</h3>
        <div class="proj-grid">
        ...
        <div class="card" style="margin-top: 30px; text-align: left;">
            <h3 style="margin-bottom: 15px;">📜 Recent Roll History</h3>
            <table>
                <thead>
                    <tr><th>#</th><th>Time</th><th>Base Used</th><th>Bet Amount</th><th>Roll</th><th>Profit BTC</th><th>Result</th></tr>
                </thead>
                <tbody>
                    ${botState.betHistory.map(b => `
                        <tr>
                            ...
                            <td style="color:#00ccff">${fmt(b.dynamicBase)}</td>
                            <td>${fmt(b.bet)}</td>
                            <td>${b.roll.toFixed(2)}</td>
                            <td class="${b.isWin ? 'win' : 'loss'}">${b.isWin ? '+' : ''}${fmt(b.profit)}</td>
                            <td class="${b.isWin ? 'win' : 'loss'}"><b>${b.isWin ? 'WIN' : 'LOSS'}</b></td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
...
loadState();
app.listen(port, () => {
    console.log(`🚀 Pro Dashboard: http://localhost:${port}`);
    runStrategy();
});
