...
let botState = {
    isRunning: false,
    isTrading: false,
    lastOrderTime: 0, // Cooldown tracker
    lastError: "None",
    currentPrice: 0,
    avgPrice: 0,
    totalContracts: 0,
...
async function htxRequest(method, path, data = {}) {
    ...
    try {
        ...
        botState.lastError = "Network/404 Error";
        return null; // Return null so logic knows the request failed
    }
}
...
async function runLogic() {
    ...
    // 2. Cooldown Guard: Don't do anything if we just placed an order in the last 15 seconds
    if (Date.now() - botState.lastOrderTime < 15000) return;
    ...
    try {
        ...
        // CRITICAL FIX: If API failed, stop here. Do NOT assume 0 positions.
        if (!posRes) {
            console.log("⚠️ API Failed to return position info, skipping this loop...");
            return;
        }
        ...
        if (pos) {
            ...
            if (botState.roi >= botState.settings.takeProfit) {
                ...
                if (res) {
                    ...
                    botState.lastOrderTime = Date.now();
                }
            } 
            // Safety Orders
            else {
                ...
                if (botState.currentPrice <= triggerPrice && botState.safetyOrdersFilled < botState.settings.maxSteps) {
                    ...
                    console.log(`📉 Adding Safety Order #${botState.safetyOrdersFilled}`);
                    const res = await htxRequest('POST', '/linear-swap-api/v1/swap_cross_order', {
                        contract_code: config.symbol, volume: vol,
                        direction: 'buy', offset: 'open', lever_rate: config.leverage, order_price_type: 'opponent'
                    });
                    if (res) botState.lastOrderTime = Date.now();
                }
            }
        } else {
            // OPEN BASE ORDER (Only if we are sure there is no position)
...
<body class="antialiased min-h-screen flex flex-col">
    <header class="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-50 px-6 h-16 flex items-center justify-between">
        <div class="flex items-center gap-2">
            ...
            <span class="font-bold tracking-tight text-lg">TradeBot<span class="text-blue-600">Stable</span></span>
        </div>
    ...
    <main class="max-w-7xl mx-auto w-full px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div class="lg:col-span-8 space-y-8">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div class="ui-card p-6"><p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Balance</p><p id="balance" class="text-2xl font-mono font-bold text-gray-800">$0.00</p></div>
            </div>
            <div class="ui-card p-8 h-64 flex flex-col items-center justify-center border-2 border-dashed border-gray-200">
                 ...
                 <p id="errorMsg" class="text-red-500 text-[10px] mt-4 uppercase font-bold">Status: OK</p>
            </div>
        </div>
    ...
    <script>
        async function refresh() {
            try {
                ...
                document.getElementById('errorMsg').innerText = 'Msg: ' + d.lastError;
