...
mongoose.connect(MONGO_URI).then(() => console.log("📦 MongoDB Connected - Balance Sync Active"));
...
let botState = {
    ...
    walletBalance: 0, // Actual cash (Static Balance)
    initialBalance: 0, // Start of session balance
    maxSafeBase: 0,
    settings: {
        baseOrder: 6000,
        autoScale: true,
...
async function runLogic() {
    // 1. ALWAYS SYNC ACCOUNT INFO (Fixes the zero balance issue)
    ...
    if (balRes && balRes.data) {
        ...
        if (usdtAccount) {
            botState.walletBalance = parseFloat(usdtAccount.static_balance);
    ...
    <header class="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
            <div class="flex items-center gap-2">
                <div class="w-9 h-9 rounded-full bg-blue-600 flex items-center justify-center shadow-md">
            ...
            <div class="flex items-center gap-4">
                <span id="statusBadge" class="text-[10px] bg-gray-100 px-3 py-1 rounded-full uppercase font-bold">Ready</span>
    ...
    <main class="max-w-7xl mx-auto w-full px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div class="lg:col-span-8 space-y-8">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div class="ui-card p-6">
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Session %</p>
                    <p id="profitPct" class="text-2xl font-mono font-bold text-blue-600">0.00%</p>
                </div>
                <div class="ui-card p-6">
            ...
            <div class="ui-card p-8 h-64 flex flex-col items-center justify-center border-2 border-dashed border-gray-200">
                 ...
                 <button onclick="resetStats()" class="mt-6 text-[10px] bg-red-500 text-white font-bold px-6 py-2 rounded-full uppercase">Reset Statistics</button>
            </div>
        </div>

        <div class="lg:col-span-4 space-y-6">
            <div class="ui-card p-6">
                <h3 class="font-bold mb-6 text-xs tracking-widest text-gray-400 uppercase">Strategy Settings</h3>
                <div class="space-y-4">
                    ...
                    <div><label class="text-xs font-bold text-gray-500 mb-1 block">Base Qty</label><input id="baseOrder" type="number" class="input-minimal"></div>
                    <div class="grid grid-cols-2 gap-4">
                        <div><label class="text-xs font-bold text-gray-500 mb-1 block">Drop %</label><input id="priceDrop" type="number" class="input-minimal"></div>
                        <div><label class="text-xs font-bold text-gray-500 mb-1 block">TP %</label><input id="takeProfit" type="number" class="input-minimal"></div>
                    </div>
                    <button onclick="saveSettings()" class="btn-primary w-full mt-4 text-xs font-bold uppercase">Update Strategy</button>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        async function refresh() {
            try {
                ...
                document.getElementById('balance').innerText = '$' + d.walletBalance.toFixed(4);
                ...
                if(firstLoad) {
                    ...
                    document.getElementById('baseOrder').disabled = d.settings.autoScale;
                    firstLoad = false;
                }
                if(d.settings.autoScale) document.getElementById('baseOrder').value = d.maxSafeBase;
                document.getElementById('mainAction').innerText = d.isRunning ? 'STOP ENGINE' : 'START ENGINE';
                document.getElementById('mainAction').className = d.isRunning ? 'btn-primary bg-red-600 py-2 px-6' : 'btn-primary py-2 px-6';
                document.getElementById('statusBadge').innerText = d.isRunning ? 'Running' : 'Stopped';
            } catch (e) {}
        }
        function toggleAuto() { document.getElementById('baseOrder').disabled = document.getElementById('autoScale').checked; saveSettings(); }
        async function toggleBot() { await fetch('/api/toggle', {method: 'POST'}); refresh(); }
        async function resetStats() { if(confirm("Reset profit?")) { await fetch('/api/reset-stats', {method: 'POST'}); refresh(); } }
