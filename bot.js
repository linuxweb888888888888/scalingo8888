...
function calculateTradeMath(side, entryPrice, currentPrice, contracts, leverage, takerFee, contractSize = 1000) {
    ...
    // Real Notional Size = Quantity * Contract Multiplier * Price
    const sizeUsd = contracts * contractSize * entryPrice;
    const currentSizeUsd = contracts * contractSize * currentPrice;
    
    const grossPnlPercent = ((currentPrice - entryPrice) / entryPrice) * 100 * sideMult;
    const margin = sizeUsd / leverage;
    
    // Gross PnL in Dollars: (Price Difference) * (Total Number of Tokens)
    ...
    const feeCost = (sizeUsd * takerFee) + (currentSizeUsd * takerFee);
    ...
    return { 
        ...
        grossRoiPct, 
        netPnlUsd, 
        netRoiPct: (netPnlUsd / margin) * 100, 
        feeCost, 
        margin 
    };
}
...
class UserTradeInstance {
            ...
            if (this.activePositions.length > 0) {
                ...
                if (signal && pos.side !== signal) {
                    ...
                    if (this.config.flipOnlyInProfit !== false) {
                        const threshold = this.config.flipThresholdPct || 0.5;
                        if (pnlPercent * FORCED_LEVERAGE >= threshold) {
    ...
    async addDcaPosition(isProfitScale = false) {
        ...
        try {
            ...
            if (isProfitScale && Number(pos.contracts) + contractsToAdd > (Number(this.walletBalance) * 2)) {
                pos.lastDcaTime = Date.now(); await this.saveState(); this.isTrading = false; return; 
            }
    ...
    startExchangeROISync() {
        setInterval(async () => {
            ...
            if (currentPrice && pos.entryPrice > 0) { 
                const math = calculateTradeMath(pos.side, pos.entryPrice, currentPrice, pos.contracts, FORCED_LEVERAGE, this.config.fees.taker, this.config.contractSize);
    ...
    (async function streamBinance() {
        ...
        while (true) {
            try {
                ...
                if (mid !== (globalMarketData.tickBuffer.length > 0 ? globalMarketData.tickBuffer[globalMarketData.tickBuffer.length - 1] : null)) {
...
async function loadAllUsers() {
    try {
        ...
        for(const u of users) {
            try {
                const worker = new UserTradeInstance(u); await worker.initialize(); activeWorkers.set(u._id.toString(), worker);
...
app.post('/api/user/keys', authMiddleware, async (req, res) => {
    try {
        ...
        if(worker) {
            ...
            if (liveTradingEnabled && !conn.success) { worker.liveTradingEnabled = false; return res.json({ error: conn.message }); }
...
<body class="antialiased min-h-screen flex flex-col selection:bg-gray-200">
    ...
    <main class="flex-grow flex flex-col justify-center">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div class="lg:col-span-8 space-y-8">
                    <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4 sm:gap-6">
                        <div class="ui-card p-5 border border-gray-100 relative"><button onclick="resetMetrics()" class="absolute top-4 right-4 text-gray-300 hover:text-red-500"><span class="material-symbols-outlined text-[16px]">refresh</span></button><p class="text-[10px] font-bold text-gray-400 uppercase mb-2">Net PnL</p><p id="netPnl" class="text-lg font-mono font-bold tracking-tight">$0.00000000</p></div>
                        <div class="ui-card p-5 border border-gray-100"><p class="text-[10px] font-bold text-gray-400 uppercase mb-2">Win Rate</p><p id="winRateDisplay" class="text-lg font-mono font-bold tracking-tight text-blue-600">0.00%</p></div>
                        <div class="ui-card p-5 border border-gray-100"><p class="text-[10px] font-bold text-gray-400 uppercase mb-2">Wallet</p><p id="marginUsed" class="text-lg font-mono font-bold tracking-tight text-gray-800">$0.00000000</p></div>
                        <div class="ui-card p-5 border border-gray-100"><p class="text-[10px] font-bold text-gray-400 uppercase mb-2">Active Qty</p><p id="activeQty" class="text-lg font-mono font-bold tracking-tight text-gray-800">0</p></div>
                        <div class="ui-card p-5 border border-gray-100"><p class="text-[10px] font-bold text-gray-400 uppercase mb-2">ROI / PNL</p><p id="activeRoi" class="text-lg font-mono font-bold tracking-tight text-gray-800">N/A</p></div>
                    ...
                    <div class="ui-card p-6 border border-gray-100">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left text-sm whitespace-nowrap">
                                <thead class="text-gray-400 uppercase text-[10px] border-b border-gray-100">
                                    <tr><th class="pb-3 px-2">Time</th><th class="pb-3 px-2">Side</th><th class="pb-3 px-2">Contracts</th><th class="pb-3 px-2">Reason</th><th class="pb-3 px-2 text-right">Gross ROI</th><th class="pb-3 px-2 text-right">Gross PnL</th><th class="pb-3 px-2 text-right">Net PnL</th></tr>
                                </thead>
        ...
        <!-- OTHER SECTIONS (ANALYTICS, LOGIN, REG, STEP, SETTINGS) -->
        <section id="view-analytics" class="view-section max-w-4xl mx-auto px-4 py-16"><div id="stat-pages" class="space-y-2 text-sm text-gray-700 font-mono"></div></section>
        ...
        <section id="view-step-history" class="view-section max-w-5xl mx-auto px-4 py-12"><h2 class="text-3xl font-bold text-center mb-10">Step History</h2><div class="ui-card p-6 border border-gray-100"><div class="overflow-x-auto"><table class="w-full text-left text-sm whitespace-nowrap"><thead class="text-gray-400 uppercase text-[10px] border-b"><tr><th>Step #</th><th>Action</th><th>Price</th><th>ROI</th><th>Time</th></tr></thead><tbody id="stepHistoryBody" class="font-mono text-xs"></tbody></table></div></div></section>
        <section id="view-settings" class="view-section max-w-lg mx-auto px-4 py-10"><div class="ui-card p-8 border border-gray-100"><h2 class="text-2xl font-bold mb-6">API Integration</h2><div class="space-y-6"><div class="flex items-center gap-3 p-4 bg-gray-50 border rounded-xl" onclick="handleLiveToggle()"><input type="checkbox" id="liveTrade" class="w-5 h-5 accent-black"><b>Enable Live Trading</b></div><input type="password" id="apiKey" class="input-minimal font-mono" placeholder="Key"><input type="password" id="apiSecret" class="input-minimal font-mono" placeholder="Secret"><button onclick="saveApiKeys()" class="btn-primary w-full py-3">Save</button><div id="key-msg" class="text-sm text-center font-medium mt-3"></div></div></div></section>
    </main>
    ...
    <script>
        ...
        function nav(v) { document.querySelectorAll('.view-section').forEach(el => el.classList.remove('active-view')); document.getElementById('view-' + v).classList.add('active-view'); currentPageView = v; pingAnalytics(true); if(v === 'dashboard' && authToken) initDashboard(); }
        ...
        async function runBacktest() { const res = await doAPI('/api/backtest', 'POST', { ticks: 5000 }); const tbody = document.getElementById('btTableBody'); tbody.innerHTML = ""; res.trades.reverse().forEach(t => { tbody.innerHTML += '<tr><td>'+t.side+'</td><td>'+t.contracts+'</td><td>'+t.exitReason+'</td><td class="text-right">$'+t.netPnl.toFixed(8)+'</td></tr>'; }); }
        ...
        const ctx = document.getElementById("mlChart").getContext("2d"); const mlChart = new Chart(ctx, { type: "line", data: { labels: [], datasets: [{ data: [], borderColor: "#000", borderWidth: 2, pointRadius: 0 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { display: false } } } });
        ...
        async function initDashboard() { if(dashLoop) clearInterval(dashLoop); dashLoop = setInterval(fetchMetrics, 1000); }
        ...
        async function fetchMetrics() {
            ...
            // DGR: (Net PnL of last 24h / Balance) * 100
            const last24h = (data.metrics.trades || []).filter(t => (Date.now() - new Date(t.timestamp).getTime()) < 86400000).reduce((s, t) => s + t.netPnl, 0);
            const dgr = data.walletBalance > 0 ? ((last24h / data.walletBalance) * 100).toFixed(2) : "0.00";
