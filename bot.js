...
const BASE_URL = "https://paradice.in/api/v1";  // Changed from crypto.games
...
async function placeBet() {
    // Paradice.in API format - adjust based on their actual API spec
    const url = `${BASE_URL}/dice/bet`;
    
    const payload = { 
        amount: Number(botState.settings.currentBet.toFixed(8)),
        payout: botState.settings.payout,
        // Paradice.in uses "over" or "under" - 2x payout typically means roll under 50
        direction: "under",
        target: 50  // 2x payout = 49.5% chance typically
    };
    ...
    try {
        ...
        botState.statusMessage = error.response?.data?.message || error.response?.data?.error || "API Error";
        return null; 
    }
}
...
async function runStrategy() {
    ...
    if (balanceData && balanceData.balance) {
        botState.stats.currentBalance = balanceData.balance;
        botState.profitProtection.safeBalance = botState.stats.currentBalance * 0.98;
        botState.settings.baseBet = calculateScaledBase(botState.stats.currentBalance);
        botState.settings.currentBet = botState.settings.baseBet;
    }
    ...
    while (true) {
        ...
        // Parse result based on Paradice.in response format
        // This needs adjustment based on actual API response
        const profit = result.profit || result.won || (result.win ? result.amount : -result.amount) || 0;
        botState.stats.netProfit += profit;
        botState.stats.currentBalance = result.balance || result.newBalance || botState.stats.currentBalance;
        ...
        botState.betHistory.unshift({ 
            ...
            bet: result.amount || botState.settings.currentBet, 
            roll: result.roll || result.result || "N/A", 
            profit: profit, 
            isWin: profit > 0, 
...
<head>
    <meta charset="UTF-8">
    <title>Dice Pro v3.3 | Paradice.in | 80% Lock</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>Dice Pro <span style="color:var(--primary)">v3.3</span> <span style="font-size:0.8rem;">for Paradice.in</span></h1>
