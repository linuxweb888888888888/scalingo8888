...
// PROFIT-OPTIMIZED CONFIGURATION (Fixed high-risk exposure)
let CONFIG = {
    coin: "BTC",
    minBet: 0.00000001,
    baseBetPercent: 0.005,        // ADJUSTED: 0.5% base to prevent rapid depletion
    maxBetPercent: 0.08,          // ADJUSTED: 8% cap (Prevents the "big chances" that cause losses)
    payout: 1.85,                 
    ...
    recoveryAggression: 1.10,     // ADJUSTED: Smoother recovery (10% vs 35%)
    seedChangeInterval: 10,
    winRateTarget: 0.54,          
    volumeMultiplier: 1.1,        // ADJUSTED: Lower volume to stay in the game longer
    compoundingRate: 1.05,        // ADJUSTED: 5% compounding
    maxDailyVolume: 0.02,         
    useProgressiveCompound: true,
    // NEW: Profit protection settings
    minPayout: 1.80,
    maxPayout: 2.00,
    recoveryBetMultiplier: 1.2,   // ADJUSTED: 20% increase in recovery (was 50%)
    winStreakBonus: 1.15           
};
...
let botState = {
    ...
    settings: {
        ...
        riskLevel: 1.1, // ADJUSTED: Lower default risk
        adaptiveMode: true,
        growthStage: 2,
        hedgeActive: true,
...
function calculateProfitOptimizedBet(isWin, currentBet, baseBet, winStreak, lossStreak, currentBalance) {
    ...
    if (CONFIG.useProgressiveCompound && isWin && winStreak > 0) {
        const compoundFactor = Math.min(2.0, 1 + (winStreak * 0.10) * CONFIG.compoundingRate); // ADJUSTED: Cap at 2.0x
    ...
    // AGGRESSIVE RECOVERY MODE - increase bets to recover faster
        ...
        if (isWin && winStreak > 0) {
            ...
            const multiplier = Math.min(2.0, 1 + (winStreak * 0.2)) * CONFIG.recoveryAggression;
            newBet = baseBet * multiplier;
        } else if (!isWin && lossStreak > 0) {
            // Aggressive increase on losses (to recover faster)
            newBet = baseBet * Math.min(1.5, 1 + (lossStreak * 0.10)) * CONFIG.recoveryBetMultiplier;
        } else {
    ...
    // Normal mode: Aggressive Anti-Martingale for profit
        ...
        if (isWin && winStreak > 0) {
            const multiplier = Math.min(2.5, 1 + (winStreak * 0.2)); // ADJUSTED: Capped multiplier
            newBet = baseBet * multiplier;
        } else if (!isWin && lossStreak > 0) {
...
function calculateOptimizedKellyProfit(balance, winProbability, payoutMultiplier, riskLevel) {
    ...
    if (kellyFraction > 0) {
        riskMultiplier *= 1.1; // ADJUSTED: Slightly less aggressive
    }
    if (botState.settings.recoveryMode) {
        riskMultiplier *= CONFIG.recoveryAggression;
    }
    
    kellyFraction = Math.max(0, kellyFraction * riskMultiplier * 0.5); // ADJUSTED: 50% Kelly (Safer)
    
    let maxCap = CONFIG.maxBetPercent;
    kellyFraction = Math.min(maxCap, Math.max(0.01, kellyFraction));
...
function checkAndOptimizeRecoveryProfit() {
    ...
    if (botState.stats.currentBalance >= botState.settings.recoveryStartBalance * 0.95 && botState.settings.consecutiveWins >= 2) {
        ...
        botState.settings.riskLevel = 1.1; // ADJUSTED
    ...
    if (recoveryProgress < -0.2 && botState.settings.consecutiveLosses > 3) {
        CONFIG.recoveryAggression = Math.min(1.3, CONFIG.recoveryAggression * 1.05); // ADJUSTED
        botState.settings.riskLevel = Math.min(1.3, botState.settings.riskLevel * 1.05); // ADJUSTED
...
function updateOptimizedStrategyProfit() {
    ...
    let newRiskLevel = 1.1; // ADJUSTED
    let newBasePercent = 0.005; // ADJUSTED
    let newMaxPercent = 0.08; // ADJUSTED
...
function validateBetProfit(betAmount, currentBalance) {
    const absoluteMax = currentBalance * 0.12; // ADJUSTED: Absolute hard cap at 12%
