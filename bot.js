...
// ==================== GLOBAL STATE ====================
let market = { last: 0, spreadPct: 0, status: 'System Starting...', totalEquity: 0, equityProfit: 0 };
...
async function htxRequest(account, method, path, data = {}) {
    ...
    try {
        ...
        if (res.data.status !== 'ok' && res.data['err_code'] !== 1046) {
        ...
        if (state.volume === 0) {
            ...
            if (!state.leverageSet) {
                await htxRequest(acc, 'POST', '/linear-swap-api/v1/swap_cross_switch_lever_rate', {
                    contract_code: config.symbol, lever_rate: config.leverage
                });
            ...
            state.lastAction = res.status === 'ok' ? `Opened ${state.direction}` : `Open Error: ${res['err_code']}`;
            state.isProcessing = false;
        } else {
            // TP/SL Logic
            if (state.roi >= config.takeProfitPct || state.roi <= config.stopLossPct) {
                state.isProcessing = true;
                ...
                if (res.status === 'ok') {
                    state.lastAction = `Closed @ ${state.roi.toFixed(2)}%`;
                    state.leverageSet = false;
                }
                state.isProcessing = false;
            } else {
                state.lastAction = `Monitoring...`;
            }
        }
    }
...
<body class="p-10"><div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-end mb-10">
        <div><h1 class="text-xl font-bold text-white uppercase tracking-tighter">Hedge Bot <span class="text-indigo-500">Auto</span></h1><p id="botStatus" class="text-[10px] text-zinc-500 font-bold uppercase"></p></div>
        <div class="flex gap-8 text-right">
            <div><p class="text-[10px] text-zinc-600 font-bold uppercase">Session Profit</p><p id="equityProfit" class="font-mono text-emerald-400 font-bold">+$0.00</p></div>
    ...
    <div class="glass rounded-3xl p-8 mb-6 text-center border-t border-indigo-500/10">
        ...
        <p class="text-zinc-400 text-xs font-mono uppercase tracking-widest">Mirror ROI: <span id="mirrorRoi" class="text-indigo-400 font-bold">0.00%</span></p>
    </div>
    <div class="grid grid-cols-2 gap-4 mb-6">
        <div class="glass rounded-2xl p-6">
            ...
            <p id="longStats" class="text-[11px] text-zinc-400 font-mono italic mb-2">Syncing...</p>
            <div class="pt-2 border-t border-white/5"><p id="longWallet" class="text-[10px] text-zinc-500 font-mono">Wallet: $0.00</p></div>
        </div>
        <div class="glass rounded-2xl p-6">
            ...
            <p id="shortStats" class="text-[11px] text-zinc-400 font-mono italic mb-2">Syncing...</p>
            <div class="pt-2 border-t border-white/5"><p id="shortWallet" class="text-[10px] text-zinc-500 font-mono">Wallet: $0.00</p></div>
        </div>
    </div>
    <button onclick="closeAll()" class="w-full py-4 bg-rose-600/20 hover:bg-rose-600 text-rose-500 hover:text-white font-bold rounded-xl text-xs uppercase tracking-widest transition-all border border-rose-500/20">Emergency Liquidate All</button>
</div>
<script>
async function closeAll(){ if(confirm("Close all?")) await fetch('/api/close',{method:'POST'}); }
setInterval(async () => {
    try {
        ...
        d.accounts.forEach((a, i) => {
            ...
            document.getElementById(pre+'Wallet').innerText = 'Balance: $' + a.wallet.toFixed(2);
