...
const Trade = mongoose.model('Trade_History', new mongoose.Schema({
    ...
    roi: Number, pnl: Number, contracts: Number, reason: String, timestamp: { type: Date, default: Date.now }
}));
...
let tickHistory = []; // BUFFER FOR RELIABLE DIRECTION
...
async function tradingLoop() {
    ...
    while (true) {
        ...
        try {
            ...
            // --- RELIABILITY LOGIC: UPDATE TICK BUFFER ---
            tickHistory.push(currentPrice);
            if (tickHistory.length > 5) tickHistory.shift(); // Keep last 5 seconds
            ...
            if (activePos) {
                ...
                if (botStatus.currentRoi >= TAKE_PROFIT || botStatus.currentRoi <= STOP_LOSS) {
                    ...
                    await Trade.create({
                        ...
                        roi: botStatus.currentRoi, pnl: botStatus.currentPnl, contracts: activePos.contracts, reason: "AUTO_EXIT"
                    });
                    botStatus.lastQty = 0;
                    tickHistory = []; // Reset history after trade
                ...
                // Only enter if we have 5 seconds of data
                if (tickHistory.length >= 5) {
                    const firstPrice = tickHistory[0];
                    const avgPrice = tickHistory.reduce((a, b) => a + b) / tickHistory.length;
                    
                    let side = null;
                    // Reliability Check: Current price must be higher than BOTH the start of the 5s window AND the average
                    if (currentPrice > firstPrice && currentPrice > avgPrice) side = 'buy';
                    if (currentPrice < firstPrice && currentPrice < avgPrice) side = 'sell';

                    if (side) {
                        const unitValue = currentPrice * 0.1;
                        const maxQty = Math.floor((botStatus.availableBalance / unitValue) * LEVERAGE * MULTIPLIER * 0.95);
...
app.get('/', (req, res) => {
    res.send(`
    <!DOCTYPE html><html><head><title>TON Bot Reliable</title><script src="https://cdn.tailwindcss.com"></script>
    ...
    <h1 class="text-2xl font-bold tracking-tighter text-blue-500 uppercase italic">TON.RELIABLE.V4</h1>
    <p class="text-[10px] text-rose-500 font-bold uppercase tracking-widest">${PAPER_TRADING ? '⚠️ PAPER MODE ACTIVE' : 'LIVE TRADING ACTIVE'}</p></div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="card p-8 border-t-2 border-blue-500">
            <div class="text-slate-500 text-[10px] uppercase font-bold mb-2">Active Qty</div>
            <div id="qty" class="text-4xl font-bold text-blue-400">0</div>
        </div>
        <div class="card p-8 border-t-2 border-slate-600">
            <div class="text-slate-500 text-[10px] uppercase font-bold mb-2">Balance</div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div class="card p-6 text-center"><div class="text-slate-500 text-[10px] mb-1 uppercase">Side</div><div id="side" class="text-xl font-bold text-white">IDLE</div></div>
        ...
        <div class="card p-6 text-center"><div class="text-slate-500 text-[10px] mb-1 uppercase">Tick Sync</div><div id="sync" class="text-xl font-bold text-blue-500">...</div></div>
    </div>

    <div class="card overflow-hidden"><table class="w-full text-left">
    <thead class="bg-slate-900/50 text-[10px] text-slate-500 font-bold uppercase tracking-widest"><tr><th class="px-6 py-4">Side</th><th class="px-6 py-4">Qty</th><th class="px-6 py-4 text-right">ROI %</th><th class="px-6 py-4 text-right">Time</th></tr></thead>
    <tbody id="history" class="text-xs"></tbody></table></div></div>

    <script>
    async function resetBaseline() {
        if(confirm("Reset paper balance to $10.00?")) { 
    ...
    <td class="px-6 py-4 text-white">\${t.contracts || 0}</td>
...
app.listen(PORT, () => console.log(`🌐 TON Bot active on port ${PORT}`));
