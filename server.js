        ...
        try {
            ...
            if (!pos.isPaper && this.liveTradingEnabled) {
                ...
                try {
                    ...
                    console.error(`[User ${this.userId}] HTX API Error:`, e.message);
                    return; 
                }
            }
                ...
                try {
                    ...
                    if (this.walletBalance > 0) {
                        if (this.initialBalance === 0) {
                            this.initialBalance = this.walletBalance;
                        }
        ...
        while (true) {
            try {
                ...
                try {
                    ...
                    mid = ((ticker.bid || ticker.last) + (ticker.ask || ticker.last)) / 2;
                } catch(wsErr) {
...
app.post('/api/user/reset-metrics', authMiddleware, async (req, res) => {
    try {
        ...
        if(worker) { 
            ...
            // RESET START BALANCE TO CURRENT POINT
...
<body class="antialiased min-h-screen flex flex-col selection:bg-gray-200">
    ...
    <header class="bg-white/80 backdrop-blur-md shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
            ...
            <nav id="nav-private" class="hidden items-center gap-6 text-sm font-medium">
                ...
                <button onclick="nav('step-history')" class="hover:text-black transition">Steps</button>
    ...
    <main class="flex-grow flex flex-col justify-center">
        ...
        <section id="view-home" class="view-section active-view w-full">
            <div class="max-w-5xl mx-auto px-4 pt-24 pb-16 text-center">
                ...
                <p class="text-lg md:text-xl text-gray-500 mb-10 max-w-2xl mx-auto leading-relaxed">TradeBot utilizes a dynamically trained logistic regression model to predict directional probabilities and executing automatically via HTX 24/7.</p>
                <button onclick="nav('register')" class="btn-primary text-base px-10 py-4 shadow-lg flex items-center gap-2 mx-auto">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div class="lg:col-span-8 space-y-8">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4 sm:gap-6">
                        <div class="ui-card p-5 relative border border-gray-100">
                            <button onclick="resetMetrics()" title="Reset Session" class="absolute top-4 right-4 text-gray-300 hover:text-red-500 transition"><span class="material-symbols-outlined text-[16px]">refresh</span></button>
                            <p class="text-[10px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">Net PnL (Session)</p>
                        ...
                        <!-- NEW GROWTH CARDS -->
                        <div class="ui-card p-5 border border-gray-100">
                            <p class="text-[10px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">PnL Growth ($)</p>
                            <p id="totalGrowthUsd" class="text-lg sm:text-xl font-mono font-bold tracking-tight">$0.0000</p>
                        </div>
                        <div class="ui-card p-5 border border-gray-100">
                            <p class="text-[10px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">Growth Percentage</p>
                            <p id="totalGrowthPct" class="text-lg sm:text-xl font-mono font-bold tracking-tight">0.00%</p>
                        </div>
                        <div class="ui-card p-5 border border-gray-100">
                    ...
                    <div class="ui-card p-6 border border-gray-100">
                        <h3 class="text-base font-bold mb-4 flex items-center gap-2">
                            <span class="material-symbols-outlined text-[20px] text-gray-500">receipt_long</span> Executions
                        </h3>
                        <div class="overflow-x-auto">
                            <table class="w-full text-left text-sm font-mono whitespace-nowrap">
                                <thead class="text-gray-400 uppercase text-[10px] tracking-wider border-b border-gray-100">
                                    <tr>
                                        <th class="pb-3 px-2">Time</th><th class="pb-3 px-2">Side</th><th class="pb-3 px-2">Size</th><th class="pb-3 px-2">Reason</th><th class="pb-3 px-2 text-right">Net PnL</th>
                                    </tr>
                                </thead>
                                <tbody id="tradeHistoryBody" class="text-xs">
                                    <tr><td colspan="5" class="py-6 text-center text-gray-400">Waiting for market shift...</td></tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                ...
                <div class="lg:col-span-4 h-fit space-y-6">
                    <div class="ui-card p-6 border border-gray-100">
                        ...
                        <div class="space-y-4">
                            <div class="flex justify-between items-center"><label class="text-sm">TP (%)</label><input type="number" id="tpPctSens" class="input-minimal w-24 text-right font-mono"></div>
                            <div class="flex justify-between items-center"><label class="text-sm">SL (%)</label><input type="number" id="slPctSens" class="input-minimal w-24 text-right font-mono"></div>
                            <button onclick="saveConfig()" class="btn-primary w-full mt-4 py-3 shadow-sm">Update Strategy</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- ANALYTICS, BACKTEST, AUTH SECTIONS (Same as First Paste) -->
        <section id="view-analytics" class="view-section max-w-4xl mx-auto px-4 py-16">
            <h2 class="text-center text-3xl font-bold mb-10">Platform Analytics</h2>
            <div class="grid sm:grid-cols-3 gap-6 mb-8">
                <div class="ui-card p-6 text-center border border-gray-100">
                    <p class="text-xs font-bold text-gray-400 mb-2">Online Now</p>
                    <p id="stat-online" class="text-4xl font-mono font-bold">0</p>
                </div>
                <div class="ui-card p-6 text-center border border-gray-100">
                    <p class="text-xs font-bold text-gray-400 mb-2">Views</p>
                    <p id="stat-views" class="text-4xl font-mono font-bold">0</p>
                </div>
                <div class="ui-card p-6 text-center border border-gray-100">
                    <p class="text-xs font-bold text-gray-400 mb-2">Uniques</p>
                    <p id="stat-uniques" class="text-4xl font-mono font-bold">0</p>
                </div>
            </div>
            <div id="stat-pages" class="ui-card p-6 border border-gray-100 font-mono text-sm space-y-2"></div>
        </section>

        <!-- BACKTEST VIEW (Same as First Paste) -->
        <section id="view-backtest" class="view-section max-w-6xl w-full mx-auto px-4 py-16">
             <h2 class="text-center text-3xl font-bold mb-10">Strategy Backtesting</h2>
             <div class="grid lg:grid-cols-4 gap-8">
                 <div class="ui-card p-6 border border-gray-100 space-y-4">
                     <div><label class="text-xs font-bold mb-1 block">Minutes</label><input type="number" id="btTicks" class="input-minimal" value="5000"></div>
                     <button onclick="runBacktest()" class="btn-primary w-full">Run Simulation</button>
                 </div>
                 <div class="lg:col-span-3 ui-card p-6 border border-gray-100">
                     <p class="text-gray-400 text-center py-20">Click run to analyze data.</p>
                 </div>
             </div>
        </section>
        ...
        <section id="view-login" class="view-section max-w-md w-full mx-auto px-4 py-20">
            <div class="ui-card p-8 border border-gray-100">
                ...
                <button onclick="doLogin()" class="btn-primary w-full">Secure Log In</button>
                <div id="login-err" class="text-red-500 text-xs mt-2 text-center"></div>
            </div>
        </section>
        ...
        <section id="view-register" class="view-section max-w-md w-full mx-auto px-4 py-20">
            <div class="ui-card p-8 border border-gray-100">
                ...
                <button onclick="doRegister()" class="btn-primary w-full">Sign Up</button>
                <div id="reg-err" class="text-red-500 text-xs mt-2 text-center"></div>
            </div>
        </section>
        ...
        <section id="view-settings" class="view-section max-w-lg mx-auto px-4 py-10">
            <div class="ui-card p-8 border border-gray-100">
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-2 mb-2"><input type="checkbox" id="liveTrade"><label class="text-sm font-bold">Enable HTX Execution</label></div>
                    <input type="password" id="apiKey" class="input-minimal" placeholder="API Key">
                    <input type="password" id="apiSecret" class="input-minimal" placeholder="API Secret">
        ...
        <section id="view-step-history" class="view-section max-w-5xl mx-auto px-4 py-12">
            <h2 class="text-3xl font-bold text-center mb-10">Active Step History</h2>
            <div class="ui-card p-6 border border-gray-100"><table class="w-full text-sm font-mono"><tbody id="stepHistoryBody"></tbody></table></div>
        </section>
    ...
    <script>
        ...
        let sessionTrackId = localStorage.getItem('rdca_visitor_id') || Math.random().toString(36).substring(2);
        ...
        function toggleAuthUI() {
            const p = document.getElementById('nav-public'), r = document.getElementById('nav-private');
            if (authToken) { p.classList.add('hidden'); r.classList.remove('hidden'); r.classList.add('flex'); }
            else { p.classList.remove('hidden'); r.classList.add('hidden'); }
        }
        ...
        async function saveApiKeys() {
            ...
            document.getElementById('key-msg').innerText = res.error || "Keys Saved.";
        }

        async function saveConfig() {
            await doAPI('/api/user/config', 'POST', { tpPct: document.getElementById("tpPctSens").value, slPct: document.getElementById("slPctSens").value });
            alert("Updated.");
        }

        async function resetMetrics() { if(confirm("Reset growth and history?")) await doAPI('/api/user/reset-metrics', 'POST'); }
        async function closeAll() { if(confirm("Close all?")) await doAPI('/api/close-all', 'GET'); }

        const mlChart = new Chart(document.getElementById("mlChart").getContext("2d"), {
            type: "line", data: { labels: [], datasets: [{ label: "SHIB", data: [], borderColor: "#000", pointRadius: 0 }] },
        ...
        async function fetchMetrics() {
            ...
            document.getElementById("netPnl").className = "text-xl font-mono font-bold " + (data.metrics.totalNetPnl >= 0 ? "text-green-600" : "text-red-600");
            document.getElementById("marginUsed").innerText = "$" + Number(data.walletBalance || 0).toFixed(4);

            // --- GROWTH UI UPDATE ---
            const growthUsdEl = document.getElementById("totalGrowthUsd");
            const growthPctEl = document.getElementById("totalGrowthPct");
            
            growthUsdEl.innerText = (data.totalPnlGrowth >= 0 ? "+" : "") + "$" + data.totalPnlGrowth.toFixed(4);
            growthUsdEl.className = "text-lg sm:text-xl font-mono font-bold " + (data.totalPnlGrowth >= 0 ? "text-green-600" : "text-red-600");
            
            growthPctEl.innerText = (data.totalGrowthPct >= 0 ? "+" : "") + data.totalGrowthPct.toFixed(2) + "%";
            growthPctEl.className = "text-lg sm:text-xl font-mono font-bold " + (data.totalGrowthPct >= 0 ? "text-green-600" : "text-red-600");
            ...
            if(data.activePositions.length > 0) {
                ...
                document.getElementById("activeRoi").className = "text-xl font-mono font-bold " + (p.exchangeROI >= 0 ? "text-green-600" : "text-red-600");
            ...
            if(data.binance) {
                mlChart.data.labels.push(""); mlChart.data.datasets[0].data.push(data.binance.mid);
                if(mlChart.data.labels.length > 100) { mlChart.data.labels.shift(); mlChart.data.datasets[0].data.shift(); }
                mlChart.update();
            }
        }
