...
const BASE_CONFIG = {
    ...
    maxContracts: 100, 
    maxDcaStepsBeforeReverse: 10, // <--- NEW SETTING
    chartTicks: 800
};
        ...
        if (activePos) {
            ...
            if (forceExitReason) {
                ...
                if (math.currentGrossRoi <= requiredRoiForDca && tickTime - (activePos.lastDcaTime || 0) >= 3000) {
                    ...
                    // LOGIC ADDED: REVERSE DIRECTION IF STEPS EXCEEDED
                    if (activePos.dcaStep >= maxDcaStepsBeforeReverse) {
                        ...
                        const revSide = activePos.side === 'long' ? 'short' : 'long';
                        let bC = parseInt(config.baseContracts) || 1;
                        const sizeUsd = bC * config.contractSize * price; 
                        activePos = { side: revSide, entryPrice: price, contracts: bC, size: sizeUsd, marginUsed: sizeUsd / FORCED_LEVERAGE, entryTime: tickTime, lastDcaTime: 0, dcaStep: 0 };
                        if (activePos.marginUsed > maxMarginUsed) maxMarginUsed = activePos.marginUsed;
                    } else {
        ...
        try {
            ...
            if (effectiveRoi >= this.config.takeProfitPct) {
                ...
                if (effectiveRoi <= requiredRoiForDca && Date.now() - (pos.lastDcaTime || 0) > 10000) {
                    ...
                    // LOGIC ADDED: IF STEPS EXCEEDED, REVERSE DIRECTION
...
app.post('/api/user/config', authMiddleware, async (req, res) => {
    ...
    pSet(maxDcaStepsBeforeReverseSens, parseInt, 'maxDcaStepsBeforeReverse'); // <--- LOGIC ADDED
        ...
        <section id="view-backtest" class="view-section max-w-6xl w-full mx-auto px-4 py-16">
            ...
            <div class="grid lg:grid-cols-4 gap-8">
                <div class="lg:col-span-1 space-y-4 ui-card p-6 border border-gray-100 h-fit">
                    ...
                    <!-- NEW BACKTEST SETTING -->
                    <div><label class="text-xs font-semibold text-gray-500 mb-1 block">Max DCA Steps Reversal</label>
                    <input type="number" id="btMaxDcaStepsBeforeReverse" class="input-minimal w-full font-mono text-gray-700 text-xs" value="10" step="1"></div>
                ...
                <div class="lg:col-span-4 h-fit space-y-6">
                    <div class="ui-card p-6 border border-gray-100">
                        ...
                        <div class="space-y-4 mt-2">
                            ...
                            <!-- NEW LIVE SETTING IN UI -->
                            <div class="flex justify-between items-center" title="If step count exceeds this, close and reverse direction">
                                <label class="text-sm font-medium text-gray-500">Max DCA Steps Reversal</label>
                                <input type="number" id="maxDcaStepsBeforeReverseSens" step="1" class="input-minimal w-24 py-1.5 text-right font-mono font-bold text-gray-700 bg-gray-50">
                            </div>
    ...
    <script>
        ...
        async function runBacktest() {
            ...
            const payload = {
                ...
                mlAverageTicks: document.getElementById('btMlAvgTicks').value, mlUseAverage: document.getElementById('btMlUseAverage').value, // Corrected ref
                ...
                maxDcaStepsBeforeReverse: document.getElementById('btMaxDcaStepsBeforeReverse').value // <--- NEW FIELD
            };
        ...
        async function saveConfig() {
            const payload = {
                ...
                maxDcaStepsBeforeReverseSens: document.getElementById("maxDcaStepsBeforeReverseSens").value // <--- LOGIC ADDED
            };
        ...
        async function fetchMetrics() {
            ...
            if(!settingsLoaded) {
                ...
                document.getElementById("baseContracts").value = Number(data.walletBalance || 0) * 10; document.getElementById("contractSize").value = data.config.contractSize || 1000;
                ...
                document.getElementById("maxDcaStepsBeforeReverseSens").value = data.config.maxDcaStepsBeforeReverse || 10; // <--- ADDED
                settingsLoaded = true;
            }
            ...
            document.getElementById("profitMultiplierSens").value = Number(data.walletBalance || 0) * 1;
            document.getElementById("maxContractsSens").value = Number(data.walletBalance || 0) * 2;
            document.getElementById("baseContracts").value = Number(data.walletBalance || 0) * 10;
