...
let botStatus = {
    ...
    currentPnl: 0,
    initialBalance: 0,
    currentBalance: 0, // Total Equity
    availableBalance: 0, // Liquid cash
    growthPnl: 0,
    growthPct: 0,
    lastQty: 0,
    errorMsg: null,
...
async function syncAccount() {
    try {
        ...
        // HTX "Total" is the Equity (Balance + PnL)
        // HTX "Free" is what's left over
    ...
    } catch (e) { botStatus.errorMsg = "Sync: " + e.message; }
}
    ...
    while (true) {
        ...
        try {
            ...
            if (pos) {
                ...
                // --- MAXIMUM AGGRESSION QTY CALCULATION ---
                // We use 97% of the AVAILABLE balance to ensure the exchange accepts the order
                const price = ticker.last;
                const buyingPower = botStatus.availableBalance * 0.97 * LEVERAGE;
                const maxQty = Math.floor(buyingPower / price);

                if (maxQty >= 1 && botStatus.availableBalance >= 0.05) {
                    const side = Math.random() > 0.5 ? 'buy' : 'sell';
                    await htx.createMarketOrder(SYMBOL, side, maxQty, undefined, {
                        'lever_rate': LEVERAGE, 'offset': 'open'
                    });
                    console.log(`FULL SEND: Opened ${maxQty} TON ${side.toUpperCase()}`);
...
app.get('/', (req, res) => {
    res.send(`
    <!DOCTYPE html><html><head><title>TON Alpha Max</title><script src="https://cdn.tailwindcss.com"></script>
    ...
    <h1 class="text-2xl font-bold tracking-tighter text-blue-500 uppercase">TON.MAX.AGGRESSION</h1>
    <p class="text-[10px] text-slate-500 font-bold uppercase">Using 97% of Wallet | 75X Leverage</p></div>
    ...
    <div class="card p-8"><div class="text-slate-500 text-[10px] uppercase font-bold mb-2">Total Account Growth</div><div id="g-pct" class="text-4xl font-bold text-emerald-400">0.00%</div></div>
    <div class="card p-8"><div class="text-slate-500 text-[10px] uppercase font-bold mb-2">Net Profit (USDT)</div><div id="g-pnl" class="text-4xl font-bold text-blue-400">+0.0000</div></div>
    <div class="card p-8"><div class="text-slate-500 text-[10px] uppercase font-bold mb-2">Starting Equity</div><div id="s-bal" class="text-4xl font-bold text-slate-400">0.0000</div></div></div>
    ...
    <div class="card p-6 text-center"><div class="text-slate-500 text-[10px] mb-1">Position TON</div><div id="qty" class="text-xl font-bold text-white">0</div></div>
    <div class="card p-6 text-center"><div class="text-slate-500 text-[10px] mb-1">Available USDT</div><div id="wallet" class="text-xl font-bold text-emerald-400">0</div></div>
    <div class="card p-6 text-center"><div class="text-slate-500 text-[10px] mb-1">Engine Time</div><div id="sync" class="text-xl font-bold text-blue-500">...</div></div></div>
    <div class="card overflow-hidden"><table class="w-full text-left">
    <thead class="bg-slate-900/50 text-[10px] text-slate-500 font-bold uppercase"><tr><th class="px-6 py-4">Side</th><th class="px-6 py-4 text-right">ROI %</th><th class="px-6 py-4 text-right">USDT PNL</th><th class="px-6 py-4 text-right">Exit</th></tr></thead>
    <tbody id="history" class="text-xs"></tbody></table></div></div>
    <script>async function update(){const res=await fetch('/api/status');const s=await res.json();
    ...
    <td class="px-6 py-4 text-right text-slate-500 uppercase font-bold">\${t.reason}</td></tr>\`).join('');}
    setInterval(update,1500);update();</script></body></html>
    `);
});

app.listen(PORT, () => console.log(`🌐 Aggressive Engine Live`));
