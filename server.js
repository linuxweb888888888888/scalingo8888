...
const CONTRACT_SIZE = 1000; // Set to 1000 as requested

// ==================== DATABASE ====================
mongoose.connect(MONGO_URI).then(() => console.log(`✅ MongoDB Connected (${SYMBOL})`));
    ...
    while (true) {
        ...
        try {
            ...
            if (activePos) {
                ...
                // REMOVED "availableBalance > 0.01" so it works with your 0.0004
                if (maxQty >= 1) {
...
app.get('/', (req, res) => {
    ...
    async function resetBaseline() {
        if(confirm("Reset all paper stats?")) { await fetch('/api/reset-baseline', { method: 'POST' }); location.reload(); }
    }
