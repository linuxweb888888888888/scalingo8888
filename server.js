...
<body class="antialiased min-h-screen flex flex-col selection:bg-gray-200">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div class="lg:col-span-8 space-y-8">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-6 gap-4 sm:gap-6">
                        ...
                        <!-- GROWTH CARDS INSERTED HERE -->
                        <div class="ui-card p-5 border border-gray-100">
                            <p class="text-[10px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">PnL Growth ($)</p>
                            <p id="totalGrowthUsd" class="text-lg sm:text-xl font-mono font-bold tracking-tight text-gray-800">$0.0000</p>
                        </div>
                        <div class="ui-card p-5 border border-gray-100">
                            <p class="text-[10px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-2">Growth Percentage</p>
                            <p id="totalGrowthPct" class="text-lg sm:text-xl font-mono font-bold tracking-tight text-gray-800">0.00%</p>
                        </div>
    ...
    <script>
        ...
        async function resetMetrics() {
            if(confirm("Are you sure you want to reset all Trade History, Net PnL, and Growth tracking? This cannot be undone.")) {
        ...
        async function fetchMetrics() {
            ...
            document.getElementById("totalGrowthUsd").className = "text-lg sm:text-xl font-mono font-bold tracking-tight " + (data.totalPnlGrowth >= 0 ? "text-green-600" : "text-red-600");
            document.getElementById("totalGrowthPct").innerText = (data.totalGrowthPct >= 0 ? "+" : "") + data.totalGrowthPct.toFixed(2) + "%";
            document.getElementById("totalGrowthPct").className = "text-lg sm:text-xl font-mono font-bold tracking-tight " + (data.totalGrowthPct >= 0 ? "text-green-600" : "text-red-600");
            ...
            if(data.metrics.totalTradesCount !== lastTradesCount && data.metrics.trades) {
                ...
                recent.forEach(t => {
                    ...
                    tbody.innerHTML += '<tr class="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition">' +
                        ...
                        '<td class="py-3 px-2 text-gray-800 font-bold">' + t.contracts.toLocaleString() + '</td>' +
                        '<td class="py-3 px-2 text-[10px] text-gray-400 font-sans tracking-wide uppercase">' + t.exitReason + '</td>' +
                        '<td class="py-3 px-2 text-right font-bold">' + t.grossRoiPct.toFixed(2) + '%</td>' +
                        '<td class="py-3 px-2 text-right font-bold">' + t.grossPnl.toFixed(4) + '</td>' +
                        '<td class="py-3 px-2 text-right font-bold">$' + t.netPnl.toFixed(4) + '</td></tr>';
                });
            }
