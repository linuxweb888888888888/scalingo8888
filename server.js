...
// ==================== DATABASE ====================
mongoose.connect(MONGO_URI).then(() => console.log(`✅ Direction Engine Active`));

const Trade = mongoose.model('Trade_History', new mongoose.Schema({
    side: String, entryPrice: Number, exitPrice: Number, roi: Number, pnl: Number, contracts: Number, reason: String, timestamp: { type: Date, default: Date.now }
}));
    ...
    while (true) {
        try {
            ...
            if (activePos) {
                ...
                botStatus.qty = activePos.contracts; // <--- UPDATE STATUS QTY
                ...
                if (botStatus.currentRoi >= TAKE_PROFIT || botStatus.currentRoi <= STOP_LOSS) {
                    ...
                    await Trade.create({ side: activePos.side, entryPrice: entry, exitPrice: currentPrice, roi: botStatus.currentRoi, pnl: botStatus.currentPnl, contracts: activePos.contracts, reason: "EXIT" });
                    botStatus.qty = 0;
                    priceHistory = []; 
...
app.get('/', (req, res) => {
    res.send(`
    <!DOCTYPE html><html><head><title>TON Bot</title><script src="https://cdn.tailwindcss.com"></script>
    ...
    <body class="p-10"><div class="max-w-5xl mx-auto">
        ...
        <div class="grid grid-cols-4 gap-6 mb-8">
            ...
            <div class="card p-6 border-t-2 border-yellow-500"><div class="text-slate-500 text-[10px] mb-2 uppercase">Contracts (Qty)</div><div id="qty" class="text-3xl font-bold text-yellow-400">0</div></div>
        ...
        <div class="card overflow-hidden"><table class="w-full text-left text-xs"><thead class="bg-slate-900 uppercase text-slate-500"><tr><th class="p-4">Side</th><th class="p-4">Contracts</th><th class="p-4 text-right">ROI%</th><th class="p-4 text-right">Time</th></tr></thead><tbody id="hist"></tbody></table></div>
    </div><script>
        setInterval(async ()=>{
